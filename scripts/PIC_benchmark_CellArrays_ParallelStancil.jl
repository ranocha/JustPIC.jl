using CUDA, MAT, CellArrays, StaticArrays, ParallelStencil
...
@parallel_indices (ix, iy) function initialize_particles!(
    pX, pY, pC, pT, pA, rad2, lx, ly, dx, dy, np
)
    if ix <= size(pX, 1) && iy <= size(pX, 2)
        for ipy in 1:np
            for ipx in 1:np
                ip = (ipx - 1) * np + ipy
                ...
                r2 =
                    (xv + pX[ix, iy][ip] * dx - lx / 2)^2 +
                    (yv + pY[ix, iy][ip] * dy - ly / 2)^2
                field(pC, ip)[ix, iy] = r2 < rad2 ? 1.0 : 0.0
                field(pT, ip)[ix, iy] = exp(
                    -(
                        (xv + pX[ix, iy][ip] * dx - lx / 2)^2 +
                        (yv + pY[ix, iy][ip] * dy - ly / 2)^2
                    ) / rad2,
...
@parallel_indices (ix, iy) function copy_particles!(
...
@parallel_indices (ix, iy) function advect!(
    pX, pY, pX_old, pY_old, pC, pC_old, pT, pT_old, pA, pA_old, Vx, Vy, dt, dx, dy
)
    if ix <= size(pX, 1) && iy <= size(pX, 2)
        nx, ny = size(pX)
        for ip in 1:prod(cellsize(pX))
            if pA_old[ix, iy][ip] == false
                continue
            end
...
@parallel_indices (ix, iy) function p2g!(C, T, pC, pT, pX, pY, pA)
    if ix <= size(pX, 1) && iy <= size(pX, 2)
        nx, ny = size(pX)
        wt = 0.0
        pc = 0.0
        ...
        for ioy in -1:1
            for iox in -1:1
                inx, iny = wrap(ix + iox, 1, nx), wrap(iy + ioy, 1, ny)
                if iox != 0 && ioy != 0 && inx == ix && iny == iy
                    continue
                end
                for ip in 1:prod(cellsize(pC))
                    if pA[inx, iny][ip] == false
                        continue
                    end
...
@parallel_indices (ix, iy) function g2p!(pT, T, T_old, pX, pY, pA, pic_amount)
    if ix <= size(pX, 1) && iy <= size(pX, 2)
        nx, ny = size(pX)
        for ip in 1:prod(cellsize(pT))
...
@parallel_indices (ix, iy) function reseed!(pX, pY, pA, pC, pT, min_pcount, max_pcount, np)
    if ix <= size(pX, 1) && iy <= size(pX, 2)
        pcount = 0
        last_idx = 1
        for ip in 1:prod(cellsize(pX))
...
@parallel_indices (ix, iy) function convert_viz!(pX_viz, pY_viz, pX, pY, pA, dx, dy)
    if ix <= size(pX, 1) && iy <= size(pX, 2)
        for ip in 1:prod(cellsize(pX))
            if pA[ix, iy][ip] == true
                xv, yv = (ix - 1) * dx, (iy - 1) * dy
...
function main()
    ...
    @parallel initialize_particles!(pX, pY, pC, pT, pA, rad2, lx, ly, dx, dy, np_d)
    @parallel p2g!(C, T, pC, pT, pX, pY, pA)
    ...
    @time while it <= nt
        ...
        @parallel g2p!(pT, T, T_old, pX, pY, pA, pic_amount)
