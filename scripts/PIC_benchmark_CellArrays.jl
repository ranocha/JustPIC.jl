    ...
    for ioy in -1:1
        for iox in -1:1
            ...
            for ip in 1:prod(cellsize(pX))
                ...
                pvx = blerp(
                    fx, fy, Vx[inx1, iny1], Vx[inx1, iny2], Vx[inx2, iny1], Vx[inx2, iny2]
                )
                pvy = blerp(
                    fx, fy, Vy[inx1, iny1], Vy[inx1, iny2], Vy[inx2, iny1], Vy[inx2, iny2]
                )
    ...
    for ip in 1:prod(cellsize(pT))
        ...
        pT_flip =
            pT[ix, iy][ip] + pT_pic - blerp(
                fx, fy, T_old[ix1, iy1], T_old[ix1, iy2], T_old[ix2, iy1], T_old[ix2, iy2]
            )
...
function main()
    ...
    CUDA.@sync @cuda threads = nthreads blocks = nblocks initialize_particles!(
        pX, pY, pC, pT, pA, rad2, lx, ly, dx, dy, np_d
    )
    ...
    @time while it <= nt
        ...
        println(" # it = $it")
        CUDA.@sync @cuda threads = nthreads blocks = nblocks copy_particles!(
            pX_old, pY_old, pC_old, pT_old, pA_old, pX, pY, pC, pT, pA
        )
        CUDA.@sync @cuda threads = nthreads blocks = nblocks p2g!(C, T, pC, pT, pX, pY, pA)
        T_old .= T
        CUDA.@sync @cuda threads = nthreads blocks = nblocks advect!(
            pX, pY, pX_old, pY_old, pC, pC_old, pT, pT_old, pA, pA_old, Vx, Vy, dt, dx, dy
        )
        CUDA.@sync @cuda threads = nthreads blocks = nblocks g2p!(
            pT, T, T_old, pX, pY, pA, pic_amount
        )
        CUDA.@sync @cuda threads = nthreads blocks = nblocks reseed!(
            pX, pY, pA, pC, pT, np0, np0 + 5, np_d
        )
        tt += dt
        # save resuts
        if it % nsave == 0
