...
function initialize_particles!(pX, pY, pC, pT, pA, rad2, lx, ly, dx, dy, np)
    ...
    for ipy in 1:np
        for ipx in 1:np
            ...
            pA[ip, ix, iy] = true
            r2 =
                (xv + pX[ip, ix, iy] * dx - lx / 2)^2 +
                (yv + pY[ip, ix, iy] * dy - ly / 2)^2
...
function main()
    ...
    @time while it <= nt
        ...
        CUDA.@sync @cuda threads = nthreads blocks = nblocks reseed!(
            pX, pY, pA, pC, pT, np * np, np * np + 5, np
        )
        tt += dt
        # save resuts
        if it % nsave == 0
